  ...
  calcWeakDc() {
    ...
    if (bigfamDcCfg > 0 || welfareDcCfg > 0) {
      for (const mm of ['mm1', 'mm2']) {
        ...
        const { yymm } = this._ret[mm];
        const calcPrice = this._ret[mm].price;
        // 복지·대가족 할인 한도의 하계는 6/1~8/31 이다. 한전 안내 원문:
        // "6월 1일부터 8월 31일까지 전기를 많이 사용하는 여름철에는 2만 원의 할인을 적용합니다."
        // 주택용 누진 요율의 하계(7/1~8/31, calcLengthUseDays)와는 기간이 다르다 — 요금표의
        // 하계 정의는 요율에만 적용되고 할인 한도에는 적용되지 않는다. 한도 금액도
        // PRICE_DC의 etc/summer 값(16,000->20,000 / 10,000->12,000 / 8,000->10,000)과 일치한다.
