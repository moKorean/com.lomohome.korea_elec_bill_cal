...
class KoreaElecBillCalculator {
  ...
  energyForecast(energy, today = null) {
    ...
    const {
      useDays, monthDays, checkMonth, checkDay,
    } = this._ret;
  ...
  calcProg() {
    ...
    this._ret.kwhWon = Math.floor(round2(kwhWonSum - diffWonSum));
    this._ret.diffWon = Math.floor(round2(diffWonSum));
  ...
  getTouRate(period) {
    ...
    const season = this._commercialSeason(this._ret.checkMonth || (new Date()).getMonth() + 1);
