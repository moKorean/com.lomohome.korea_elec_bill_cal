...
class KoreaElecBillCalculator {
  ...
  calcProg() {
    ...
    for (const mm of ['mm1', 'mm2']) {
      ...
      const climatePrice = calcPrice.adjustment[1];
  ...
  calcFuelWon() {
    ...
    const fuelPrice = calcPrice.adjustment[2];
  ...
  calculate(energy, today = null) {
    try {
      ...
      // Non-residential flat (계절별 단일요율) tariffs use a separate engine.
   ...
   */
  getSimpleBill(energy) {
    const result = this.calculate(energy);
    return {
